...
def logop(z):
  for word in z + '\n':
     ...
     time.sleep(0.01)
def maogit():
  os.system("xdg-open https://github.com/beltoknet/")
def ext():
...
def more():
  ...
  {acl}[{gcl} 6{bcl}.{acl} ]{ycl} GITHUB {acl}[{bcl} Betok Project {acl}]
...
def more2():
  ...
  {acl}[{gcl} 7{bcl}.{acl} ]{ycl} GITHUB {acl}[{bcl} Betok Project {acl}]
  {acl}[{rcl} 0{bcl}.{acl} ]{rcl} EXIT {acl}[{bcl} ×××{acl} ]
  """.format(acl=acl,bcl=bcl,gcl=gcl,ycl=ycl,rcl=rcl,ccl=ccl)
  try:
...
def main2():
  try:
    ...
    {acl}[{gcl} 5{bcl}.{acl} ]{ycl} GITHUB {acl}[{bcl} Betok Project {acl}]
...
def main():
  try:
    ...
    {acl}[{gcl} 4{bcl}.{acl} ]{ycl} GITHUB {acl}[{bcl} Betok Project {acl}]
    {acl}[{rcl} 0{bcl}.{acl} ]{rcl} EXIT{acl} [{rcl} ××× {acl}]
    """.format(acl=acl,bcl=bcl,gcl=gcl,ycl=ycl,rcl=rcl,ccl=ccl)
    while True:
